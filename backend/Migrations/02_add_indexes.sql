...
create index if not exists "IX_Users_Name_Surname" on "Users"("Name", "Surname");
...
create index if not exists "IX_Tasks_StatusId" on "Tasks"("StatusId");
...
create index if not exists "IX_Tasks_UpdatedAt" on "Tasks"("UpdatedAt");

-- Composite index for searching project tasks by status (often used together)
create index if not exists "IX_Tasks_ProjectId_StatusId" on "Tasks"("ProjectId", "StatusId");

-- Composite index for searching user tasks by status
create index if not exists "IX_Tasks_OwnerId_StatusId" on "Tasks"("OwnerId", "StatusId");

-- Composite index for searching tasks with deadline in project
create index if not exists "IX_Tasks_ProjectId_Deadline" on "Tasks"("ProjectId", "Deadline") where "Deadline" is not null;
...
create index if not exists "IX_TaskAssignees_TaskId" on "TaskAssignees"("TaskId");
create index if not exists "IX_TaskAssignees_UserId" on "TaskAssignees"("UserId");

-- Composite unique index to check if user is already assigned to task
create unique index if not exists "IX_TaskAssignees_TaskId_UserId" on "TaskAssignees"("TaskId", "UserId");

-- Composite index for searching all user tasks
create index if not exists "IX_TaskAssignees_UserId_TaskId" on "TaskAssignees"("UserId", "TaskId");
...
create index if not exists "IX_ProjectMembers_RoleId" on "ProjectMembers"("RoleId");
...
create index if not exists "IX_ProjectMembers_UserId_ProjectId" on "ProjectMembers"("UserId", "ProjectId");

-- Index for searching project members by role
create index if not exists "IX_ProjectMembers_ProjectId_RoleId" on "ProjectMembers"("ProjectId", "RoleId");
...
create index if not exists "IX_Comments_TaskId_CreatedAt" on "Comments"("TaskId", "CreatedAt");
...
-- Indexes for foreign keys
create index if not exists "IX_TaskHistory_TaskId" on "TaskHistory"("TaskId");
create index if not exists "IX_TaskHistory_UserId" on "TaskHistory"("UserId");

-- Index for sorting by change date
create index if not exists "IX_TaskHistory_ChangedAt" on "TaskHistory"("ChangedAt");

-- Composite index for searching task history with chronological sorting
create index if not exists "IX_TaskHistory_TaskId_ChangedAt" on "TaskHistory"("TaskId", "ChangedAt");

-- Index for searching changes by field
create index if not exists "IX_TaskHistory_FieldName" on "TaskHistory"("FieldName");

-- Composite index for searching user changes in task
create index if not exists "IX_TaskHistory_TaskId_UserId" on "TaskHistory"("TaskId", "UserId");
...
create unique index if not exists "IX_Favorites_UserId_EntityType_EntityId" on "Favorites"("UserId", "EntityType", "EntityId");

-- Index for searching favorites by entity type
create index if not exists "IX_Favorites_EntityType_EntityId" on "Favorites"("EntityType", "EntityId");

-- Index for sorting by addition date
create index if not exists "IX_Favorites_CreatedAt" on "Favorites"("CreatedAt");

-- Composite index for searching user favorites by type
create index if not exists "IX_Favorites_UserId_EntityType" on "Favorites"("UserId", "EntityType");
...
create index if not exists "IX_Tasks_ProjectId_Null" on "Tasks"("ProjectId") where "ProjectId" is null;